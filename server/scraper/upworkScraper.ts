...
export async function scrapeUpworkJobs(options: ScrapeOptions): Promise<Job[]> {
    ...
    const extractedJobs = await page.evaluate(() => {
      ...
      jobElements.forEach((element) => {
        try {
          ...
          // Multiple selector attempts for description
          ...
          // Extract budget/rate with multiple selectors
          const budgetElement =
...
export function generateMockJobs(count: number): Job[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    jobs.push({
      ...
      url: `https://www.upwork.com/jobs/~${randomUUID().replace(/-/g, "").slice(0, 18)}`,
    });
  }
